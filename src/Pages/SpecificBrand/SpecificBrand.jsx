...
export default function SpecificBrand() {
    ...
    async function getRelatedProducts(brandId) {
        setLoading(true);
        try{
        let {data} = await axios.get(`https://ecommerce.routemisr.com/api/v1/products/?brand[in]=${brandId}`)
        console.log(data);
    ...
    return (
        <>
            ...
            <>
            <Link to={'/brands'} className='mt-6 flex items-center justify-center hover:scale-120 transition-all duration-200 space-x-2 bg-secondaryColor size-7 rounded-full text-white'>
